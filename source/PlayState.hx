...
class PlayState extends MusicBeatState
{
	...
	var funnyFloatyBoys:Array<String> = ['dave-angey', 'bambi-3d', 'expunged', 'bambi-unfair', 'bambi-piss-3d', 'bandu', 'unfair-junker', 'exbungo',
	...
	override public function create()
	{
		...
		{
			case 'disruption':
				dialogue = CoolUtil.coolTextFile(Paths.txt('charts/disruption/disruptDialogue'));
			case 'applecore':
				dialogue = CoolUtil.coolTextFile(Paths.txt('charts/applecore/coreDialogue'));
			case 'disability':
				dialogue = CoolUtil.coolTextFile(Paths.txt('charts/disability/disableDialogue'));
			case 'wireframe':
				dialogue = CoolUtil.coolTextFile(Paths.txt('charts/wireframe/wireDialogue'));
			case 'algebra':
				dialogue = CoolUtil.coolTextFile(Paths.txt('charts/algebra/algebraDialogue'));
		}
		...
		shaggyVoice = isShaggy && [
			...
			'roofs', 'supernovae', 'glitch', 'master', 'cheating', 'unfairness', 'kabunga', 'recursed', 'exploitation', 'applecore'
	...
	public function createBackgroundSprites(bgName:String, revertedBG:Bool):FlxTypedGroup<BGSprite>
	{
		...
		switch (bgName)
		{
			...
			case 'POOP':
				...
				altSong = Song.loadFromJson('applecore/alt-notes' + diff);

				var bg3:BGSprite = new BGSprite('bg2', -350, -375, Paths.image('backgrounds/void/applecore/yeah'), null, 1, 1, true, true);
				bg2.setGraphicSize(Std.int(bg.width * 1.75));
				sprites.add(bg3);
				voidShader(bg3);
				bg3.active = false;
				add(bg3);
				
				var bg:BGSprite = new BGSprite('bg', -600, -200, Paths.image('backgrounds/void/applecore/hi'), null, 1, 1, true, true);
				bg.setGraphicSize(Std.int(bg.width * 1.75));
				sprites.add(bg);
				voidShader(bg);
				add(bg);

				var bg2:BGSprite = new BGSprite('bg2', -600, -200, Paths.image('backgrounds/void/applecore/poop'), null, 1, 1, true, true);
				bg2.setGraphicSize(Std.int(bg.width * 1.75));
				sprites.add(bg2);
				voidShader(bg2);
				bg2.active = false;
